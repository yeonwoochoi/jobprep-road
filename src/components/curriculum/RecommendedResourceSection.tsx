...
const ResourceIcons: Record<ResourceType, (props: SVGProps<SVGSVGElement>) => any> = {
  course: (props) => (
    <svg {...props} viewBox="0 0 20 20" fill="currentColor">
      <path
        d="M2 6a2 2 0 012-2h12a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V6zm4 2v6h8V8H6z"
        clipRule="evenodd"
        fillRule="evenodd"
      />
    </svg>
        ...
        {recommendedResources.map((resource) => {
          ...
          return (
            <a
              ...
              className="group relative flex flex-col justify-between rounded-lg border border-neutral-200 bg-neutral-50 p-5 transition-shadow duration-200 hover:shadow-md"
            >
              <div>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                  <div className="inline-flex items-center gap-x-1.5 rounded-full bg-neutral-200 px-2.5 py-1 text-xs font-medium text-neutral-700">
                    <Icon className="h-4 w-4" />
                    <span>{label}</span>
                  </div>
                  {resource.platform && (
                    <span className="text-sm text-neutral-500">{resource.platform}</span>
                  )}
                </div>
                <h3 className="mt-4 text-base font-semibold text-neutral-800">{resource.title}</h3>
                <p className="mt-2 text-sm/6 text-neutral-600">{resource.description}</p>
              </div>
